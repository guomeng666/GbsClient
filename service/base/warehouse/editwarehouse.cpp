...
void EditWarehouse::Execute(GbsSession &session)
{
    //封包处理
    ...
    rootObj.insert("WarehouseName",QJsonValue(session.getRequestData("WarehouseName")));
    rootObj.insert("Enabled",QJsonValue(session.getRequestData("Enabled")));
