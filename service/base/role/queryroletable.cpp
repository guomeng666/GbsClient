...
QueryRoleTable::QueryRoleTable()
{

}

void QueryRoleTable::Execute(GbsSession &session)
{
    //封包处理
    ...
    for(int i = 0; i < data.size(); ++i ) {
        ...
        QStringList rowData;
        rowData << QString::number(row.value("ID").toInt())<<row.value("Name").toString()<<row.value("Permission").toString()\
