...
QuerySellerByIdentityID::QuerySellerByIdentityID()
{

}

void QuerySellerByIdentityID::Execute(GbsSession &session)
{
    //封包处理
    ...
    for(int i = 0; i < data.size(); ++i ) {
        ...
        rowData << QString::number(row.value("ID").toInt())<<row.value("Name").toString()\
                   <<row.value("IdentityID").toString()<<row.value("Address").toString()\
                   <<row.value("Phone").toString()<<row.value("BankType").toString()\
                   <<row.value("BankID").toString()<<row.value("Creater").toString()\
